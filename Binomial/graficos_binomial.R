...
data_Mp2=tibble(n=0:20,media2_infec_geracao) # média 0.3
teorico_Mp2 = tibble(x=0:20, y=0.3^x)
g2b = 
  ggplot() + 
  geom_line(data=data_Mp2, aes(x=n, y=media2_infec_geracao,colour = "Média amostral"),size=1)+
  geom_point(data=teorico_Mp2, aes(x=x, y=y, colour = "Média teórica"))+
  ylab("") +
...
######## Gráficos variância: mu = p*n, var = n*p*(1-p)
